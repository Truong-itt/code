...
def get_links(url):
    ...
    while True:
        ...
        if i % 50 == 0:
            print(i)
        response = requests.get(f"{url}/timeline/6/trang-{i}.htm", cookies=cookies, headers=headers)
        if response.status_code != 200:
            break
        soup = BeautifulSoup(response.content, 'html.parser')
        links.extend([f"{url}{a['href']}" for a in soup.select('a.box-category-link-title')])
...
def info(links_in_journal):
    ...
    for link in links_in_journal:
        try:
            ...
            content = ' '.join([p.text for p in soup.select('div.detail-content.afcbc-body p')])
            new_data = {
                'title': title,
    ...
    print('Saved to JSON')
links_in_journal = get_links(url)
